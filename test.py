...
def main():
    ...
    if opt.resize_short_edge is None:
        print(f"You haven't specified the resize_short_edge, so the maximum resolution is set to {opt.max_resolution}")

    opt.device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

    # Support two test modes: single image test and batch test (through a text file)
